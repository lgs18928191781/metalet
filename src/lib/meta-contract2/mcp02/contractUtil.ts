...
import { TokenUnlockContractCheckFactory, TOKEN_UNLOCK_TYPE } from './contract-factory/tokenUnlockContractCheck';
...
export class ContractUtil {
  ...
  public static init(config?: ContractConfig) {
    if (config) {
      ...
      this.transferCheckCodeHashArray = getTokenTransferCheckCodeHashArray().map((v) => new Bytes(v));
      this.unlockContractCodeHashArray = getTokenUnlockContractCheckCodeHashArray().map((v) => new Bytes(v));
