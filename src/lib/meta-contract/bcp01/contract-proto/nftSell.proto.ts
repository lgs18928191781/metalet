...
export function getNftID(script: Buffer) {
  return toHex(script.slice(script.length - NFT_ID_OFFSET, script.length - NFT_ID_OFFSET + NFT_ID_LEN));
}
...
export function getSatoshisPrice(script: Buffer): BN {
  ...
  return BN.fromBuffer(
    script.slice(script.length - SATOSHIS_PRICE_OFFSET, script.length - SATOSHIS_PRICE_OFFSET + SATOSHIS_PRICE_LEN),
    { endian: 'little' }
  );
}
...
export function getTokenIndex(script: Buffer): BN {
  ...
  return BN.fromBuffer(
    script.slice(script.length - TOKEN_INDEX_OFFSET, script.length - TOKEN_INDEX_OFFSET + TOKEN_INDEX_LEN),
    { endian: 'little' }
  );
}
...
export function getGenesis(script: Buffer) {
  ...
  return script.slice(script.length - GENESIS_OFFSET, script.length - GENESIS_OFFSET + GENESIS_LEN).toString('hex');
}

export function getCodeHash(script: Buffer) {
  if (script.length < CODEHASH_OFFSET) return '';
  return script.slice(script.length - CODEHASH_OFFSET, script.length - CODEHASH_OFFSET + CODEHASH_LEN).toString('hex');
}
