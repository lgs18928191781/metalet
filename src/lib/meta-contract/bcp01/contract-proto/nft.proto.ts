...
export function getTotalSupply(script: Buffer): BN {
  ...
  return BN.fromBuffer(
    script.slice(script.length - TOTAL_SUPPLY_OFFSET, script.length - TOTAL_SUPPLY_OFFSET + TOTAL_SUPPLY_LEN),
    { endian: 'little' }
  );
}
...
export function newDataPart({
  ...
  if (totalSupply) {
    totalSupplyBuf = totalSupply.toBuffer({ endian: 'little', size: TOTAL_SUPPLY_LEN }).slice(0, TOTAL_SUPPLY_LEN);
  }
