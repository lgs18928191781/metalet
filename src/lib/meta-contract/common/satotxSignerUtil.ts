...
export async function getRabinDatas(
  ...
  for (let i = 0; i < sigReqArray.length; i++) {
    for (let j = 0; j < sigReqArray[i].length; j++) {
      ...
      if (j == 0) {
        checkRabinMsgArray = Buffer.concat([checkRabinMsgArray, Buffer.from(sigInfo.byTxPayload, 'hex')]);
...
export async function selectSigners(signerConfigs: SignerConfig[], signerNum: number, signerVerifyNum: number) {
  let _signerConfigs = signerConfigs.map((v) => Object.assign({}, v));
  if (_signerConfigs.length < signerNum) {
    throw new CodeError(ErrCode.EC_INVALID_ARGUMENT, `The length of signerArray should be ${signerNum}`);
  }
  ...
  for (let i = 0; i < _signerConfigs.length; i++) {
    ...
    let ret = new Promise((resolve: ({ url, idx }: { url: string; idx: number }) => void, reject) => {
      ...
      retPromises[j]
        ...
        .catch((e) => {
          ...
          if (failedCnt + successCnt == retPromises.length) {
            reject(new CodeError(ErrCode.EC_INNER_ERROR, `Less than ${signerVerifyNum} successful signer requests`));
          }
        });
    }
  });
