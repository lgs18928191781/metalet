...
import { CONTRACT_TYPE, P2PKH_UNLOCK_SIZE, PLACE_HOLDER_PUBKEY, PLACE_HOLDER_SIG, isNull } from '../common/utils';
...
export class FtManager {
  ...
  private async _mint({
    ...
    let { genesisContract, genesisTxId, genesisOutputIndex, genesisUtxo } = await this._prepareMintUtxo({ sensibleId });
  ...
  private async _prepareMintUtxo({ sensibleId }: { sensibleId: string }) {
    ...
    let genesisUtxo = await this._getMintUtxo(genesisContract.getCodeHash(), genesisTxId, genesisOutputIndex);
    if (!genesisUtxo) {
  ...
  private async _getMintUtxo(codehash: string, genesisTxId: string, genesisOutputIndex: number): Promise<FtUtxo> {
    ...
    let genesisUtxos = await this.api.getFungibleTokenUnspents(codehash, originGenesis, this.purse.address.toString());
    ...
    if (!unspent) {
      ...
      let issueUtxos = await this.api.getFungibleTokenUnspents(codehash, issueGenesis, this.purse.address.toString());
  ...
  public async transfer({
    ...
    let txHex = txComposer.getRawHex();
    const inSats = transferCheckTxComposer.getTx().inputs.reduce((pre, input) => pre + input.output.satoshis, 0);
    const outSats = transferCheckTxComposer.getTx().outputs.reduce((pre, output) => pre + output.satoshis, 0);
