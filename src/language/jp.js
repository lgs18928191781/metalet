export default {
  ...
  wallet: {
    ...
    welcome: 'Metaletへようこそ！',
    welcome_2: 'Metaletは、ブロックチェーン世界への接続を支援します。',
    welcome_3: '初めまして.',
    term: '利用規約',
    begin: '開始',
    ...
    create_wallet_notice_4:
      'ブラウザのストレージがクリアされたり、デバイスが破損または紛失した場合にウォレットを復元するには、これらの単語が必要になります。',
    ...
    mnemonic_notice_4:
      'ブラウザを変更したり、コンピュータを移動したりする場合は、このアカウントにアクセスするためにニーモニック単語が必要になります。 それらを安全で秘密の場所に保存しましょう。',
    ...
    private_notice_4:
      'ブラウザを変更したり、コンピュータを移動したりする場合は、このアカウントにアクセスするためにこの秘密鍵が必要になります。 それらを安全で秘密の場所に保存します。',

    adv_options: '高度なオプション',
    options_notice: '次のオプションがわからない場合は、変更しないでください。',
    passphrase: 'パスフレーズ:(オプション）',
    passphrase_notice_1:
      'パスフレーズは、資産をさらに保護することができます。 ニーモニックが盗まれた場合でも、パスフレーズが公開されていなければ、資産は安全です。',
    passphrase_notice_2:
      'パスフレーズが設定されている場合は、ニーモニックワードをインポートするときにこれを入力する必要があります。',
    passphrase_notice_3:
      'パスフレーズが設定されているが、パスフレーズを入力せずに12のニーモニックワードがインポートされた場合、パスフレーズのあるウォレットは復元されず、パスフレーズのない新しいウォレットが作成されます。',
    ...
    passphrase_import_notice_2:
      '間違ったパスフレーズを入力すると、復元されたウォレットは元のパスフレーズと同じウォレットにはなりません。',
