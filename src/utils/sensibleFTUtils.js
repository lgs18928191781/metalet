...
const notDefaultSigners = {
  ...
  'https://satotx2.showpay.top': {
    satotxApiPrefix: 'https://satotx2.showpay.top',
    satotxPubKey:
  ...
  'https://satotx3.showpay.top': {
    satotxApiPrefix: 'https://satotx3.showpay.top',
    satotxPubKey:
...
utils.merge = async function (senderWif, purseWif, genesis, codehash, utxoCount, signers) {
  ...
  if (signers && signers.length > 0) param.signers = signers;
...
utils.transfer = async function (
  ...
  if (signers && signers.length > 0) ftParams.signers = signers;
  if (signerSelecteds && signerSelecteds.length > 0) ftParams.signerSelecteds = signerSelecteds;
