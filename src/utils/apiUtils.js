...
apiUtils.getTokenPrice = function (genesis) {
  if (genesis) return httpUtils.get(`${host}/token_price/${genesis}`);
  else return httpUtils.get(`${host}/token_price`);
};
...
apiUtils.listNftByGenesis = function (codehash, genesis, address, cursor, size) {
  return httpUtils.get(
    `${config.sensibleUrl}/nft/utxo-data/${codehash}/${genesis}/${address}?cursor=${cursor}&size=${size}&appid=sensilet`
  );
};
