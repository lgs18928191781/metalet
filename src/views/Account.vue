<template>
              ...
              <div class="balance">
                ...
                <div v-else class="balance-container">
                  <span>
                    ...
                    <span class="decimal" v-if="bsvAsset.showBalance.decimal.length > 0"
                      >.{{ bsvAsset.showBalance.decimal }}</span
                    >
                    <!--                                    {{ bsvAsset.balance.total / Math.pow(10, bsvAsset.decimal) }} -->
                    <span style="margin-left: 4px; font-size: 0.85em">{{ bsvAsset.name }}</span>
                  </span>
  ...
  <a-modal v-model:visible="showTransPanel" :closable="false" @cancel="clearTransferInfo">
    <div class="trans-info-container">
      <div class="title">Send {{ isTransBSV ? 'BSV' : transInfo.name }}</div>
      ...
      <div v-else>
        ...
        <div class="input-container" :class="{ 'has-error': inputErrorNotice !== '' }">
          <a-input
            ...
            :placeholder="
              $t('account.input_amount', [transType === 'BSV' ? 'BSV' : transInfo.unit || transInfo.symbol])
            "
          />
        ...
        <div class="notice">
          ...
          <div class="fee">
            <div class="key">{{ $t('account.fee') }}:</div>
            <div class="amount">
  ...
  methods: {
    ...
    async initAppList() {
      try {
        ...
        if (temp === 'can_not_show') this.canShowApp = false;
        else this.appList = JSON.parse(temp);
      } catch (e) {
        console.error(e);
      }
    ...
    clearTransferInfo() {
      ...
      if (this.$refs.addressInput) this.$refs.addressInput.reset();
    },
    ...
    async transfer() {
      ...
      switch (this.transType) {
        case 'BSV':
          ...
          if (amount > this.transInfo.balance.total) return antMessage.error(this.$t('account.balance_not_enough'));
