<template>
  ...
  <a-modal v-model:visible="isShowDelete" @ok="deleteCurrentAccount">
    <p>
      {{ $t('setting.current_account_name') + ': ' }} <span style="font-weight: bold">{{ $store.getters.alias }}</span>
    </p>
    <p>
      {{ $t('setting.current_account') + ': ' }} <span style="font-weight: bold">{{ $store.getters.addressShow }}</span>
    </p>
  ...
  <a-modal v-model:visible="isShowChangePassword" @ok="changePassword">
    ...
    <a-input
      ...
      v-model:value="changePasswordData.oldPwd"
      :placeholder="$t('setting.input_password')"
    />
    <a-input
      type="password"
      style="margin-top: 16px"
      v-model:value="changePasswordData.newPwd"
      :placeholder="$t('setting.input_new_password')"
    />
    <a-input
      type="password"
      style="margin-top: 16px"
      v-model:value="changePasswordData.reNewPwd"
      :placeholder="$t('setting.input_new_password_again')"
    />
  </a-modal>
</template>
...
export default {
  ...
  methods: {
    async openEdit() {
      ...
      await nextTick();
      if (this.$refs.editAliasInputEle) this.$refs.editAliasInputEle.focus();
    },
    openChangePassword() {
      this.isShowChangePassword = true;
    },
    handleOk() {
      if (!this.editAlias) return;
      if (this.editAlias.length > 12) return antMessage.error(this.$t('account.alias_max_limit'));
      if (this.editAlias.length < 1) return antMessage.error(this.$t('account.alias_min_limit'));
    ...
    changeNetworkConfirm() {
      ...
      antModal.confirm({
        ...
        content: this.$t('setting.change_network_notice', [config.isTestnet ? 'Mainnet' : 'Testnet']),
        onOk() {
    ...
    async deleteCurrentAccountConfirm() {
      ...
      await nextTick();
      if (this.$refs.deleteInputEle) this.$refs.deleteInputEle.focus();
    },
    ...
    changePassword() {
      ...
      try {
        walletManager.changePassword(
          this.changePasswordData.oldPwd || 'SatoWallet#2021',
          this.changePasswordData.newPwd || 'SatoWallet#2021'
        );
