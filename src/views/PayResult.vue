<template>
  <div class="panel-container">
    <div class="panel">
      <div style="color: green; font-size: 24px; margin-top: 20px">Success!</div>
        ...
        <div class="receive-item" v-for="item in to">
          ...
          <div class="info fee-usd">
            ...
            <div class="value" style="color: #666" v-if="item.usd">
              <span>{{ item.usd }}</span
              ><span class="unit" style="margin-left: 4px">USD</span>
            </div>
...
export default {
  ...
  data() {
    ...
    if (!routerData) routerManager.gotoHome();
  ...
  async mounted() {
    ...
    if (txInfo) {
      ...
      if (!this.bsvPrice) this.bsvPrice = (await apiUtils.getBsvPrice()).data;
      if (this.bsvPrice) this.feeUsd = ((this.fee / Math.pow(10, 8)) * this.bsvPrice).toFixed(2);
    }
  },
  methods: {
    async commit() {
