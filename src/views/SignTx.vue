...
export default {
  ...
  async mounted() {
    ...
    let list = this.list;
    if (!list || list.length <= 0) return this.cancel('list is empty');

    let txHexMap = {};
    for (let i = 0; i < list.length; i++) {
      if (txHexMap[list[i].txHex]) continue;
      ...
      for (let j = 0; j < txDetail.length; j++) {
        ...
        if (txDetail[j].type === txUtils.txType.SENSIBLE_FT) {
          ...
          if (typeof data.tokenAmount === 'string') data.tokenAmount = parseInt(data.tokenAmount);
          temp.type += `(${data.tokenName.replaceAll('/u000', '')})`;
          temp.amount =
            data.decimalNum > 0
              ? (data.tokenAmount / Math.pow(10, data.decimalNum)).toFixed(data.decimalNum)
              : data.tokenAmount;
