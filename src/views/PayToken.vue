    ...
    <div class="panel scrolled">
      ...
      <div v-if="tokenInfo" style="margin-top: 20px">
        ...
        <div class="receive-container" :class="{ 'not-enough': tokenInfo.rest < 0 }" style="margin-top: 20px">
          <div class="info">
            <div class="key">From:{{ mineAddress }}</div>
            <div class="value">
              <CoinShow
                :value="tokenInfo.balance"
                is-bold-amount
                :big-unit="tokenInfo.unit"
                :decimal="tokenInfo.decimal"
                :fixed="tokenInfo.decimal"
                show-big-unit
              />
            </div>
          </div>
          ...
          <div class="info">
            ...
            <div class="value" v-else style="color: #cc0000">{{ tokenInfo.name }} not enough</div>
          </div>
        </div>
        <div class="receive-container" :class="{ 'not-enough': isBsvNotEnough }">
          <div class="info">
            ...
            <div v-if="fee != null" class="value">
              <div v-if="isBsvNotEnough" style="color: #cc0000">BSV not enough</div>
              <div v-else>
...
export default {
  ...
  data() {
    ...
    if (routerData) {
      if (routerData.receivers) data.receivers = routerData.receivers;
      else
        data.receivers = [
          { amount: routerData.amount, address: routerData.address, addressShow: showLongString(routerData.address) },
        ];
      data.broadcast = routerData.broadcast;
      data.genesis = routerData.genesis;
      data.codehash = routerData.codehash;
      if (routerData.utxo) data.utxo = routerData.utxo;

      if (routerData.origin) origin = routerData.origin;
    } else {
  ...
  methods: {
    async checkUtxo() {
      ...
      let utxoCount = await tokenManager.sensibleFt.getUtxoCount(
        this.tokenInfo.genesis,
        this.tokenInfo.codehash,
    ...
    async getFeeUsd() {
      ...
      if (!isNaN(this.fee) && total < this.fee) this.isBsvNotEnough = true;
      ...
      for (let i = 0; i < this.receivers.length; i++) {
        if (price) {
          this.receivers[i].usd = (
            (this.receivers[i].amount / Math.pow(10, this.tokenInfo.decimal)) *
            price.USDT
          ).toFixed(2);
        } else this.receivers[i].usd = '-';
      }
    ...
    cancel() {
      if (origin) {
        ...
        window.close();
      } else routerManager.gotoHome();
    },
    async commit() {
      try {
        this.isPaying = true;

        let { txid, txHex, routeCheckTxHex, tx } = await tokenManager.transfer(
          this.receivers,
          this.broadcast,
          tokenInfo,
          this.utxo,
        ...
        if (typeof e === 'string') return antMessage.error(e);
        ...
        } else if (e) antMessage.error(e.message);
        else antMessage.error('unknown error');
      } finally {
        this.isPaying = false;
      }
    },
