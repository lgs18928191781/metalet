<template>
  ...
  <div class="panel" v-else-if="step === 2">
    ...
    <a-input
      ...
      :placeholder="$t('wallet.input_password_again')"
      type="password"
    />
  ...
  methods: {
    async createMnemonic() {
      return new Promise((resolve) => {
        ...
        if (!mnemonic) mnemonic = walletManager.createMnemonic();
      ...
      try {
        if (this.step === -1) {
          ...
          if (this.password && this.password.length > 0) {
            if (this.password !== this.rePassword) return antMessage.warning(this.$t('wallet.password_error'));
          }
          if (!this.password) {
            //  没设置则使用 默认密码
        ...
        } else antMessage.error(this.$t('wallet.save_error'));
      } catch (e) {
...
function goNextPage() {
  ...
  } else routerManager.goto('/account');
}
</script>
