...
export default createStore({
  ...
  mutations: {
    initAccount(state) {
      ...
      if (state.account) state.account.addressShow = showLongString(state.account['address' + config.hostFix]);
      ...
      if (accountList) {
        state.accountList = accountList.map((item) => {
          ...
          item.isCurrent =
            state.account && state.account['address' + config.hostFix] === item['address' + config.hostFix];
          return item;
        });
      }
    },
  ...
  actions: {
    ...
    async refreshAllToken({ commit, state }) {
      ...
      state.totalTokenValue = state.tokenList
        .reduce((value, item) => {
          if (item.usd) return value + parseFloat(item.usd);
          return value;
        }, 0)
        .toFixed(2);
