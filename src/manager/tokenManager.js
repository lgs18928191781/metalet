...
tokenManager.refreshLocalTokenData = async function () {
  ...
  for (let i = 0; localList && i < localList.length; i++) {
    ...
    if (allTokenTable[localList[i].genesis]) {
      ...
      if (!tokenInfo.fixed) tokenInfo.fixed = tokenInfo.decimal;
      if (!tokenInfo.unit) tokenInfo.unit = tokenInfo.name;
      if (!tokenInfo.logo) tokenInfo.logo = '/img/empty-token.png';
      if (!tokenInfo.network) tokenInfo.network = 'mainnet';
...
tokenManager.listUserTokens = async function (showAll = false) {
  ...
  if (showAll) onlyShowAdded = false;
  ...
  if (result.code === 0) {
    ...
    for (let i = 0; i < tempList.length; i++) {
      if (tokenTable[tempList[i].genesis]) tempList[i].logo = tokenTable[tempList[i].genesis].logo; // 给数据加上logo
    ...
    if (onlyShowAdded) {
      ...
      for (let i = 0; localList && i < localList.length; i++) {
        ...
        } else localList[i].balance = tempList[index].balance;
      ...
      tokenList = localList;
    } else tokenList = tempList;
  } else {
    //查询接口返回错误时
    tokenList = getLocalTokenList();
    for (let i = 0; i < tokenList.length; i++) {
      tokenList[i].balance = await sensibleFtUtils
        .getBalance(tokenList[i].genesis, tokenList[i].codehash, address)
        .catch((e) => 0);
    }
  }
...
tokenManager.getTokenInfo = async function (genesis, codehash) {
  ...
  //查询余额
  if (tokenInfo) {
    tokenInfo.balance = await sensibleFtUtils.getBalance(
      tokenInfo.genesis,
      tokenInfo.codehash,
...
tokenManager.transfer = async function (receivers, broadcast, { genesis, codehash }, utxo, signers, signerSelecteds) {
  ...
  return sensibleFtUtils.transfer(
    genesis,
    codehash,
    wif,
    wif,
