...
function getRootKey(fromSeed = false) {
  if (mRootKey) return mRootKey;
  if (fromSeed) mRootKey = bsv.Bip32.fromSeed(walletManager.getSeedFromLocked());
  else mRootKey = bsv.Bip32.fromSeed(mnemonicUtils.getSeedFromMnemonic(walletManager.getMnemonic()));
  return mRootKey;
}
...
walletManager.getAccountMode = function (lockInfo) {
  if (!lockInfo) lockInfo = JSON.parse(localStorage.getItem('lockInfo'));
  if (!lockInfo) {
    return '';
  }
  if (lockInfo.isSinglePrivateKey) return 'account.mode_single_key';
...
walletManager.getSeedFromLocked = function () {
  ...
  if (!mPassword) {
    if (!bg.passwordAesTable[lockInfo.address]) throw new Error('Unlock Wallet First');
...
walletManager.getMainAddress = function () {
  if (mMainAddress !== '') return mMainAddress;
...
walletManager.getWifAndPubKey = function (path = '/0/0') {
  ...
  return {
    ...
    pubKey: bsv.PubKey.fromPrivKey(privateKey).toString(),
    address: bsv.Address.fromPrivKey(privateKey).toString(),
  };
};
...
walletManager.getBsvBalance = async function (address) {
  if (!address) address = walletManager.getMainAddress();
...
walletManager.payArray = async function (receivers, broadcast, wif = null) {
  ...
  if (!wif) wif = walletManager.getMainWif();
  ...
  return {
    rawHex: txComposer.getRawHex(),
    fee: txComposer.getUnspentValue(),
    txid: txComposer.getTxId(),
    tx: txComposer.tx,
    isInvalid: txComposer.getFeeRate() < config.fee,
  };
};
