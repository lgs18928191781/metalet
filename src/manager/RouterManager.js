...
routerManager.goto = function (url, data) {
  if (config.debug) console.log('goto', url);
  //修改链接上的参数显示
  let needGoto = url;
  ...
  if (url !== needGoto) {
    ...
    if (data) sessionStorage.setItem('go_for_url_data', JSON.stringify(data));
  }
...
routerManager.gotoDebug = function () {
  ...
  if (needGoto) routerManager.goto(needGoto, JSON.parse(localStorage.getItem('debug_need_goto_data') || '{}'));
  else routerManager.gotoHome();
};
