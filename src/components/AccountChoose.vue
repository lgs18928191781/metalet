<template>
  <a-dropdown
    ...
    :class="{ hide: !$store.state.account && (!$store.state.accountList || $store.state.accountList.length <= 0) }"
    :trigger="['click']"
  >
    <div class="current-account" @click.prevent>
      <div v-if="$store.state.account != null && current !== '/create'" style="display: flex; align-items: center">
        <HeadPicture
          ...
          style="width: 40px; height: 40px; margin-right: 10px; padding: 8px"
        />
    ...
    <template #overlay>
      <a-menu
        v-if="$store.state.accountList && $store.state.accountList.length > 0"
        style="max-height: 60vh; overflow-y: auto"
      >
        <a-menu-item
          v-for="item in $store.state.accountList"
          @click="choose(item)"
          class="account-item"
  ...
  methods: {
    ...
    choose(item) {
      if (item.address === this.$store.getters.address) {
        if (this.current === '/create') routerManager.gotoHome();
        return;
      }
      walletManager.reload();
