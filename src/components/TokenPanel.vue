<template>
  <div class="panel">
    ...
    <div class="list" v-else-if="$store.state.tokenList.length > 0">
      <div
        class="item"
        v-for="(item, index) in editTokenMode ? editList : $store.state.tokenList"
        :class="{ open: item.open && !editTokenMode }"
      >
  ...
  <a-modal v-model:visible="showTransPanel" :closable="false">
    <div class="trans-info-container" v-if="transInfo">
      <div class="title">Send {{ transInfo.name }}</div>
      ...
      <div v-else>
        <div style="margin-top: 8px">Address: {{ transAddress }}</div>
        <div class="input-container" :class="{ 'has-error': inputErrorNotice !== '' }">
          <a-input
            v-model:value="transAmount"
            @change="transAmountChange"
            ref="transAmountInputEle"
        ...
        <div class="notice">
          <div class="balance" v-if="transInfo">
            ...
            <div class="amount">{{ transBalance }} {{ transUnit }}</div>
...
export default {
  ...
  methods: {
    ...
    toggleItem(item) {
      if (!this.editTokenMode) item.open = !item.open;
    },
    ...
    async openTokenList() {
      if (this.showTokenType === 'all') return;
      ...
      if (list && list.length > 0) {
        list.forEach((item) => {
          ...
          if (item.name) item.searchWord += '/' + item.name.toLowerCase();
          if (item.unit) item.searchWord += '/' + item.unit.toLowerCase();
        });
      }
    ...
    refreshSearch() {
      if (this.inputSearch.length <= 0) this.showTokenList = this.baseTokenList;
      else
    ...
    editActionAdd(index) {
      if (!this.editTokenMode || !this.editList || this.editList.length < 1) return;
      if (index <= 0) return;

      if (!this.editList[index] || !this.editList[index - 1]) return;

      if (this.editList[index - 1].topped && !this.editList[index].topped) return;
    ...
    editActionSub(index) {
      if (!this.editTokenMode || !this.editList || this.editList.length < 1) return;
      if (index >= this.editList.length - 1) return;
      if (!this.editList[index] || !this.editList[index + 1]) return;
      if (this.editList[index].topped && !this.editList[index + 1].topped) return;
    ...
    editActionTop(index) {
      ...
      if (temp.topped) {
        ...
        if (i >= 0) this.editList.splice(i, 0, temp);
        else this.editList.push(temp);
      } else {
        temp.topped = true;
        this.editList.splice(index, 1);
    ...
    async addCustomToken() {
      if (!this.customToken.genesis || !this.customToken.codehash) return;

      let result = await apiUtils.getTokenInfo(this.customToken.genesis, this.customToken.codehash);
      if (!result || result.code !== 0) return antMessage.error(this.$t('account.token_error'));
    ...
    async sendToken(item) {
      ...
      await nextTick();
      if (this.$refs.addressInput) await this.$refs.addressInput.focusInput();
    },
    receive(item) {
      // this.showQr = true
    ...
    transAmountChange(value) {
      ...
      if (this.transAmount > this.transBalance) return (this.inputErrorNotice = this.$t('account.balance_not_enough'));
      else this.inputErrorNotice = '';
    ...
    async calcTransFee() {
      if (this.transFeeLoading) return;
      ...
      try {
        ...
        if (
          tokenInfo.notDefaultSigners ||
          this.transInfo.genesis === '54256eb1b9c815a37c4af1b82791ec6bdf5b3fa3' ||
        ...
        let fee = await tokenManager.sensibleFt.getTransferEsitimate(
          this.transInfo.codehash,
          this.transInfo.genesis,
          [
            {
    ...
    async transfer() {
      ...
      if (!amount || isNaN(amount)) return antMessage.error(this.$t('account.amount_error'));

      amount = Math.round(parseFloat(amount) * Math.pow(10, this.transInfo.decimal));
      if (amount <= 0) return antMessage.error(this.$t('account.amount_error_2'));
      if (amount > this.transInfo.balance) return antMessage.error(this.$t('account.balance_not_enough'));

      this.btnLoading = true;

      //获取token utxo数
      let utxoCount = await tokenManager.sensibleFt.getUtxoCount(
        this.transInfo.genesis,
        this.transInfo.codehash,
        walletManager.getMainAddress()
      );
      //获取bsv utxo数
