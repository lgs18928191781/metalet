<template>
  <div class="top">
    <div class="title">
      <img
        v-if="width < 500 && showAccountChoose"
      ...
      <div v-if="isTestnet" class="testnet">Testnet</div>
    </div>
    ...
    <span
      class="version"
      :class="{ 'right-little': showSetting, 'has-new': $store.getters.hasNewVersion }"
      @click="newVersion()"
    >
...
export default {
  ...
  beforeCreate() {
    ...
    if (routerManager.getCurrentPage() === '/') {
      if (config.debug) return routerManager.gotoDebug();
  ...
  mounted() {
    ...
    if (!config.isTestnet) this.checkDataVersion();
  },
  methods: {
